...
class StatsFullScreen extends HookWidget {

  final String? status;

  const StatsFullScreen({super.key,this.status});
  ...
  Widget build(BuildContext context) {
    ...
    useEffect((){
      ...
      // Combine Created + Received view
      if (status == Constants.statuses[0] || status == Constants.statuses[1]) {
        selectedStatus.value = null; // means show both Created & Received
      } else {
        selectedStatus.value = status; // show a single status
      }
        ...
        fetchPage: (int pageKey) async {
          ...
          if (res is ApiData<PagedResponse<OrderResponseModel>>) {
            ...
            final filtered = (selectedStatus.value == null)
                ? pageResult.data.where((order) {
              final s = order.status?.toLowerCase();
              return s == Constants.statuses[0].toLowerCase() ||
                  s == Constants.statuses[1].toLowerCase();
            }).toList()
                : pageResult.data;
